...
**Darwin Trader** (`AntaresAndBharani/darwin-trader`) is an algorithmic trading platform engineered for MetaTrader 5 (MT5) with a modern Android companion application (Jetpack Compose), an asynchronous Terminal User Interface (Textual TUI), a high-performance Python strategy execution and backtesting engine, and a FastAPI gateway providing real-time 1Hz WebSocket telemetry streaming and REST controls.

The platform is purpose-built to comply with **Darwinex Zero** institutional evaluation metrics, prioritizing capital preservation, strict daily drawdown constraints, dynamic volatility-adjusted position sizing, and algorithmic consistency to maximize the Darwinex D-Score.

```mermaid
graph TD
    subgraph PresentationTier ["Presentation Layer (Mobile & Terminal Clients)"]
        ...
        subgraph TuiClient ["Terminal User Interface Client (tui/)"]
            TextualApp["Textual TUI Application (tui.app, tui.screens, tui.widgets)"]
            TuiClientWrapper["DarwinApiClient (httpx.AsyncClient & WebSocket Poller)"]
        end
    end
    ...
    subgraph GatewayTier ["Gateway & Communications Layer (api_gateway/)"]
        ...
        GatewayLock["Concurrency Lock (_state_lock: threading.Lock)"]
    end
    ...
    subgraph BrokerTier ["Broker Infrastructure & Execution Targets"]
        ...
        MockMT5["Platform-Independent Mock Engine (In-Memory Order Book & Positions)"]
    end

    subgraph QualityTier ["Quality & Agentic Governance Subsystems"]
        MaestroE2E["Maestro Declarative E2E Suites (e2e/flows: 01-05)"]
        PytestSuite["Backend Pytest Suite (api_gateway/tests, strategy_engine/tests)"]
    ...
    ViewModels -->|Collects UI State| ComposeUI
    ViewModels -->|HTTP REST / WebSocket Requests| RetrofitClient
    ...
    Connector -->|Live Windows Execution| LiveMT5
...
| **Backend Framework** | FastAPI & Uvicorn | FastAPI `>=0.100.0`, Uvicorn `>=0.22.0` | Asynchronous REST gateway and 1Hz WebSockets push streaming |
...
| **Broker Integration** | MetaTrader 5 Python SDK | `MetaTrader5 >= 5.0.45` (Windows condition) | Win32 IPC bridge to MT5 trading terminals (`terminal64.exe`) |
| **Terminal UI (TUI)** | Textual & Rich | Textual `>=0.70.0`, Rich `>=13.0.0` | Asynchronous, keyboard-driven ANSI dashboard for headless servers and desktop |
...
- **Live Mode (`mock_mode = False`)**: Establishes live Win32 IPC with the MetaTrader 5 terminal process on Windows. Requires valid broker credentials.
- **Mock Simulation Mode (`mock_mode = True`)**: Activates an in-memory execution engine with virtual tickets, synthetic account balance, and simulated tick fills. Allows full backend and UI verification on Linux, macOS, and headless CI runners where the proprietary MT5 Windows binary cannot execute.

---

## Layer Boundaries & Clean Architecture (Domain, Data, Presentation/UI separation of concerns)

The Darwin Trader ecosystem enforces strict **Clean Architecture** boundaries and **Unidirectional Data Flow (UDF)**. Dependencies strictly point inward toward domain models and business invariants.
...
graph RL
    subgraph PresentationLayer ["Presentation Layer (Mobile UI, TUI, ViewModels, Routers)"]
        ...
        FastApiRouters["FastAPI Routers (api_gateway/routes_*)"]
    end

    subgraph DataLayer ["Data & Infrastructure Layer (Networking, Brokers, DTOs)"]
    ...
    subgraph DomainLayer ["Domain Layer (Business Logic, Entities, Algorithms, Contracts)"]
        DomainEntities["Domain Entities (models.py: Candle, Position, AccountInfo, TradeSignal)"]
        ...
        DomainContracts["Android Domain Contracts (UseCases, Repository Interfaces)"]
    end
...
The Domain layer is the heart of the platform. It defines enterprise trading models, risk guardrails, and technical analysis algorithms:

- **Responsibilities:**
  - **Core Financial Entities (`strategy_engine/models.py`)**: Defines domain entities (`Candle`, `TradeSignal`, `Position`, `AccountInfo`, `StrategyState`, `ConnectionStatus`) and domain enumerations (`OrderType`, `SignalType`, `StrategyStatus`, `ConnectionState`). Models are strictly immutable or validated using Pydantic v2.
  - **Risk Guardrails (`strategy_engine/risk_manager.py`)**: Enforces institutional evaluation rules:
    1. Maximum open position limits (`max_open_positions`).
    2. Hard daily floating drawdown caps (`max_daily_drawdown_pct`).
    3. Minimum free margin buffers (`free_margin >= balance * 0.05`).
    4. Volatility-adjusted and stop-loss-based dynamic position sizing (`calculate_lot_size`).
  - **Technical Indicator Algorithms (`strategy_engine/strategy_base.py`)**: Pure mathematical implementations of Exponential Moving Average (`calculate_ema`), Relative Strength Index (`calculate_rsi`), and Average True Range (`calculate_atr`).
  - **Quantitative Simulation Engine (`strategy_engine/backtester.py`)**: Bar-by-bar historical backtesting simulation calculating equity curves, Win Rates, Profit Factors, and Maximum Drawdowns without network or broker dependencies.
  - **Android Domain Contracts**: Clean Kotlin domain models, Use Cases (`GetAccountTelemetryUseCase`, `ExecuteOrderUseCase`, `TriggerKillSwitchUseCase`, `AuthenticateAccountUseCase`), and repository interfaces (`AccountRepository`, `StrategyRepository`).
...
- **Responsibilities:**
  - **MetaTrader 5 Hardware Abstraction (`strategy_engine/mt5_connector.py`)**:
    ...
    - Provides thread-safe order execution (`execute_order`), position querying (`get_open_positions`), and account telemetry extraction (`get_account_info`).
  ...
  - **Mobile Networking (`android/app/src/main/java/com/darwintrader/app/data/`)**:
    - `ApiService`: Retrofit interface defining REST endpoints for account status, positions, telemetry, strategy commands, and Darwinex stats.
    - DTO Serialization: Maps backend JSON structures to strongly-typed data transfer objects (`AccountInfo`, `Position`, `AccountConnectRequest`, `AccountConnectResponse`).
...
- **Responsibilities:**
  ...
  - **Terminal User Interface (`tui/`)**:
    - `DarwinTraderApp`: Textual application managing full-screen layouts, reactive message loops, keybindings (`F1`, `F2`/`C`, `P`, `K`, `R`, `Q`), and modal dialogs.
    - `HeaderBar`: Top telemetry bar with dual-glyph accessibility badges (`[● CONNECTED]`, `[○ DISCONNECTED]`), latency metrics, and server info.
    - `AccountConnectModal`: Dialog for dynamic MT5 re-authentication with inline error diagnostics.
...
│   │       │       ├── data/                     # Data layer (Networking, Models, Repositories)
│   │       │       │   ├── api/
...
│   │       │       │   ├── model/                # Pure domain entities (Account, Position, Telemetry)
│   │       │       │   ├── repository/           # Domain repository contracts (AccountRepository, StrategyRepository)
│   │       │       │   └── usecase/              # Interactors (GetTelemetryUseCase, ExecuteKillSwitchUseCase)
...
│   ├── models.py                                 # Core trading models, signals, positions, enums
...
│   └── tests/                                    # Strategy and risk test suite
...
│       └── test_tui.py                           # Pilot tests verifying rendering and keybindings
...
class DashboardViewModel(
    ...
    val uiState: StateFlow<DashboardUiState> = combine(
        accountRepository.getAccountInfoStream(),
        accountRepository.getPositionsStream(),
        strategyRepository.getStrategyStatusStream()
...
fun DashboardContent(
    ...
    // Pure declarative layout and rendering
}
```

### 3. Repository Pattern & Single Source of Truth (SSOT)

Repositories mediate between network APIs, WebSockets, and local state:
- All data access is routed through repository implementations (`AccountRepositoryImpl`, `StrategyRepositoryImpl`).
- Repositories encapsulate network caching, offline mock fallbacks, and data transformation from network DTOs to clean domain models.
- Upstream consumers (ViewModels and Use Cases) remain decoupled from underlying transport details (Retrofit vs Ktor vs WebSockets).

### 4. Concurrency & Dual-Lock Synchronization Pattern (Gateway & Strategy Engine)

Algorithmic trading demands strict concurrency guarantees to prevent race conditions during order dispatch, position closure, and configuration updates:

1. **State Lock (`api_gateway.routes_strategy._state_lock`)**:
   - A module-level `threading.Lock()` synchronizing high-level route mutations (updating `global_config`, transitioning `current_status`).
2. **Connector Lock (`strategy_engine.mt5_connector.MT5Connector._lock`)**:
   - A reentrant lock (`threading.RLock()`) protecting internal mock positions, balance counters, and direct Win32 IPC invocations.
3. **Non-Blocking Thread-Pool Delegation for MT5 Win32 IPC**:
   - `mt5.initialize()` and `mt5.login()` can block for **2 to 10 seconds** when resolving remote broker servers over Win32 sockets.
...
### 5. Hardware Abstraction & Mock Simulation Strategy (Platform-Independence Pattern)
...
- When `mock_mode = True` or when the MT5 library is absent, the connector seamlessly routes all order operations to an internal in-memory execution engine.
- Simulates realistic ticket generation (`_mock_ticket_counter`), spreads, margin allocation (`$200.00` per position), and floating PnL calculation.
- Guarantees 100% test coverage and CI verification on headless Ubuntu GitHub Actions runners.

### 6. Emergency Kill-Switch Pattern (Fail-Safe Portfolio Liquidation)
...
### 7. Quantitative Signal-Driven Trading Pattern (Strategy Base & Risk Decorator)
...
5. **Position Sizing**: If validated, `RiskManager.calculate_lot_size()` dynamically computes lot volume based on stop-loss distance and account risk percentage before dispatching to `MT5Connector.execute_order()`.
...
### 8. Hybrid WebSocket-First Push Streaming with Polling Fallback

Real-time telemetry architecture balances low-latency responsiveness with network resilience:
- **Primary Push Channel**: Clients (Android / TUI) open a persistent WebSocket connection to `/ws/live`, receiving 1Hz JSON snapshots containing balance, equity, floating PnL, margin, D-Score, and position lists.
...
3. **Non-Blocking Main Thread & Structured Concurrency**:
   ...
   - In FastAPI, blocking Win32 MT5 SDK calls (`mt5.initialize`, `mt5.login`, `mt5.order_send`) must never execute directly inside `async def` route handlers. They must be dispatched via `asyncio.to_thread()`.
...
6. **Strict Concurrency Protection**:
   - Shared mutable state in Python (active positions, running status, global config) must be guarded by synchronization locks (`_state_lock` or `MT5Connector._lock`).
7. **Pydantic v2 Schema Hygiene**:
   - Models handling external API responses must configure `model_config = ConfigDict(extra="ignore")` or explicit field aliases to prevent runtime crashes caused by unexpected JSON payload keys.
...
| **UI Logic in Domain Layer** | Importing Android UI widgets or formatters into domain entities or risk calculations. | Keep domain models 100% pure Kotlin / Python data classes with zero platform UI imports. |
...
1. **Automated Verification**:
   ...
   - Backend unit tests pass: `pytest api_gateway/tests strategy_engine/tests`
